...
对于 Llama-70B（$L=80$, $H_{kv}=8$, $D=128$, $S=32768$）：

$$M_{\text{KV}} = 2 \times L \times H_{kv} \times D \times S \times 2 = 2 \times 80 \times 8 \times 128 \times 32768 \times 2 \approx 10.74 \text{ GB}$$

在 IB 50 GB/s 下约需 215 ms——对短 prompt 是显著开销，对长 prompt（如 S=32K, prefill 本身需数百 ms）可接受。
...
本节将 本系列[第四篇](../part-4/) 的 Roofline 模型扩展到**单请求延迟**的精确估算，为后续章节的**并发场景**提供基础输入。
...
$S=2048$（长 prompt）: $\text{FLOPs}_{\text{prefill}} \approx 2×70×10^9×2048 + 4×2048^2×64×128×80 \approx 2.87×10^{14} + 1.10×10^{13} \approx 2.98×10^{14}$。$T_{\text{compute}} \approx 2.98×10^{14}/742×10^{12} \approx 0.40$ s = **400 ms**。$T_{\text{mem}} \approx (140+0.08)/3.35e3 \approx 42$ ms。**Compute-bound** → TTFT ≈ 400 ms。
...
对于 Llama-70B on H100（**单卡, 无 TP**）：$T_{\text{decode}} \approx 140\text{GB}/3.35\text{TB/s} \approx 42$ ms。TPOT ≈ **42 ms per token**（单请求, 无并发, 无 TP）。

> 注：单卡 42 ms 是理论下限。实际部署通常用 TP=4，将每卡权重降到 35 GB，TPOT 降到 ~10.4 ms（见 CH 7.1）。下文 CH 6.3 的并发表基于**单卡无 TP**的基线，CH 9 的完整推演基于 TP=4。

**Ascend 910C 侧（单卡 vs H100 单卡）**：910C 单卡 HBM 带宽 3.2 TB/s（H100 3.35 TB/s 的 96%），容量 64 GB（H100 80 GB 的 80%）。单卡 TPOT ≈ 140/3.2 ≈ 44 ms（vs H100 42 ms, 差距 ~5%）。TP=4 时每卡 35 GB，TPOT ≈ 35/3.2 ≈ 11 ms（vs H100 TP=4 的 10.4 ms）。**两平台在相同 TP 配置下延迟接近。**
...
以 Llama-70B on H100 **单卡（无 TP）** 为例，不同 $B$ 下的 TPOT：

| $B$ | TPOT | 总吞吐 | 注 |
|---|---|---|---|
| 1 | ~42 ms | 24 tok/s | 纯 memory-bound (权重 I/O 140 GB) |
| 4 | ~44 ms | 91 tok/s | 权重 I/O 仍主导 (KV 增量 ~1 GB 可忽略) |
| 16 | ~55 ms | 291 tok/s | KV cache 增量开始显著 |
| 64 | ~100 ms | 640 tok/s | 接近 compute-bound, KV + 计算并重 |
...
**显存约束**：MoE 总参数量极大。DeepSeek-V3 671B，BF16 总权重 = 1342 GB——远超任何单卡（H100 80 GB, A100 80 GB, 910C 64 GB）。
...
- [ ] **算单 token decode 延迟**（Roofline, 来自 本系列[第四篇](../part-4/)）
