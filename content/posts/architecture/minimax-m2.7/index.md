...
tags = ['moe', 'attention', 'model-architecture', 'gqa', 'mtp', 'minimax']
series = ['architecture']
summary = 'M2.7（229.9B/9.8B 激活）的核心不在规模而在自我进化范式。本期拆解五代演进脉络（Text-01 → M2.7）、62层 Full Attention + GQA + 256 MoE 的设计空间、单 token 6.1 TFLOPs / 48.8GB KV Cache 的计算分析，以及 attention 和 MoE gate 的算子级拆解。'
+++

# MiniMax-M2.7 架构深度拆解

> 范围：M2.7（229.9B/9.8B 激活）· 撰写：2026-06-09
...
- **CH 1**：MiniMax-Text-01 → M2.7 五代演进脉络
- **CH 2**：整体架构、超参表、参数分解与 FLOPs 分析
- **CH 3-6**：四大核心创新——Full Attention 回归、GQA + QK Norm、MoE 路由、MTP
- **CH 7**：支撑项——192K 上下文 / FP8 量化 / 自进化框架
- **CH 8**：源码映射汇总
- **CH 9**：总结

**适合**：先看 CH 0–2 拿全貌，再按兴趣挑 CH 3–6；CH 8 适合边读边查。
...
M2 论文 §2.2.2 明确指出（来源：paper §2.2.2）：
...
以下超参来自 `config.json`（来源：MiniMaxAI/MiniMax-M2.7）与论文 §2.1 的交叉验证：
...
- **层数 62 > V4-Flash 43**（来源：config.json → num_hidden_layers）：M2.7 选择了「更深更窄」的设计（3072 vs 4096），与 M2 论文强调的「mini activations」原则一致
- **GQA ratio = 6**（48H/8KV）：平衡了 KV cache 开销（8/48 = 17%）与 attention 质量
- **rope_theta = 5M**（来源：config.json → rope_theta）：Llama-3 的 10 倍，为 192K 上下文提供更细粒度的远程位置编码
- **k = 8, 无 shared expert**（来源：config.json）：与 V4 的 k=6 + 1 shared expert 形成对比——M2.7 选择了「纯 routed」策略，靠 routing bias 保障负载均衡
- **intermediate_size = 1536 < hidden_size = 3072**（来源：config.json）：每个 expert 的 SwiGLU 中间维度仅为隐藏维度的 50%，这是一个激进的小 expert 设计

### 2.4 参数分解——229.9B 的构成

（来源：参数计算基于 config.json + `modeling_minimax_m2.py` 代码审查）
...
- 62 层合计：62 × (44.1M + 113.5M) ≈ **9.8B**（来源：paper §2.1，与代码核算一致）
...
对比：V4-Flash 在 1M 上下文下 KV cache ~17 GB（得益于 HCA m=128 的 T/128 压缩）。M2.7 的 Full Attention 在 192K（1/5 上下文）下 KV cache 已经是 V4 的 2.9×。
...
**论文揭示的具体失败模式**（来源：paper §2.2.2 Table 2/3）：
...
论文 Table 2（来源：paper §2.2.2）给出了 pre-training 阶段的对比。关键差距集中在长上下文检索任务：
...
M2.7 的 Attention 类（来源：`modeling_minimax_m2.py:L236-L314`）：
...
（来源：`modeling_minimax_m2.py:L259-L314`）
...
源码中的 `repeat_kv` 函数（来源：`modeling_minimax_m2.py:L153-L163`）将 8 个 KV 头复制 6 次匹配 48 个 Q 头。

### 4.2 QK Norm per_layer

M2.7 在 attention 计算前对 Q 和 K 分别做 RMSNorm（来源：`modeling_minimax_m2.py:L256-L258`）：
...
`qk_norm_type = per_layer` 意味着每一层有独立的 QK Norm 参数（来源：config.json → qk_norm_type）。
...
M2.7 的 MoE 路由由 `MiniMaxM2SparseMoeBlock` 实现（来源：`modeling_minimax_m2.py:L103-L131`）：

1. **Gate 投影**：`self.gate = nn.Linear(3072, 256)` — 将隐藏状态映射到 256 维专家分数（来源：L109）
2. **sigmoid 评分**：`routing_weights = F.sigmoid(router_logits)` — 使用 sigmoid 而非 softmax（来源：L113）
3. **Routing Bias**：`scores = routing_weights + self.e_score_correction_bias` — 加载偏置（来源：L114）
4. **Top-8**：`torch.topk(scores, 8)` — 选 top-8（来源：L115）
5. **权重归一化**：`weights /= weights.sum(dim=-1)` — 非 softmax，仅归一化（来源：L117）
6. **Expert dispatch**：`one_hot` + `index_add_`（来源：`MiniMaxM2Experts.forward` L68-L101）
...
（来源：`modeling_minimax_m2.py:L111`）

`e_score_correction_bias` 是 `register_buffer` 而非 `nn.Parameter`——不参与梯度更新。它在训练时由外部 RL/平衡机制动态调整（类似 DeepSeek V3.2 的 aux-loss-free bias），命中率高的 expert 的 bias 减小，命中率低的 bias 增大。

### 5.3 sigmoid 评分 vs softmax

`sigmoid` 评分的关键特性（来源：config.json → scoring_func = sigmoid）：
...
`shared_intermediate_size = 0`（来源：config.json）意味着 M2.7 不设共享专家。所有 token 的输出完全依赖 top-8 routed expert 的组合。这与 V4 的「1 shared + 6 routed」形成对比。M2.7 的策略是靠 256 个 expert 的细粒度和 routing bias 的平衡机制来覆盖通用能力。

![MoE 路由](fig-5.1-moe-routing.svg)

### 5.5 算子级拆解：route_tokens_to_experts

（来源：`modeling_minimax_m2.py:L112-L118`）
...
**Step 1 为什么用 sigmoid？**（来源：config.json → scoring_func=sigmoid）
...
**Step 2: routing bias 的更新机制**（来源：`modeling_minimax_m2.py:L111`）
...
config.json 中 `num_experts_per_tok=8`。选择 8 而非 6（V4-Flash）或 2（Mixtral 8×7B）的权衡：
...
M2.7 的 MTP 实现直接位于 `MiniMaxM2Model.forward` 中（来源：`modeling_minimax_m2.py:L471-L540`），没有独立的 `MTPBlock` 类——而是通过循环创建辅助预测头：
...
### 6.3 与 DeepSeek V3 MTP 的差异

| 维度 | V3 | M2.7 |
|---|---|---|
| MTP 模块数 | 1 | **3** |
| 每模块层数 | 1 (main) + 1 (mtp) | **1** |
| 推理时用途 | speculative decoding | speculative decoding |

M2.7 选择 3 个模块（每个 1 层）而非 V3 的 1 个模块（2 层），体现了「浅而多」的设计哲学。

![MTP 架构](fig-6.1-mtp-architecture.svg)

---

## CH 7. 支撑项——192K 上下文 / FP8 / 自进化

### 7.1 192K 上下文

- `max_position_embeddings = 204,800`（来源：config.json）
...
（来源：config.json → quantization_config）
...
M2.7 的训练流程引入「自我进化」范式（来源：paper §3）：
...
3. **MTP × 3 > MTP × 1**：M2.7 选择 3 个浅 MTP 模块（每个 1 层），在 speculative decoding 中提供更多候选 draft token。

### 9.2 已知局限

- **推理速度**：全注意力在长上下文下性能退化显著（实测 45.6 TPS vs 声称 100 TPS）
- **推理能力弱于代码能力**：中文评测指出「文字强但推理弱」
- **M2/M2.5 Apache 2.0 → M2.7 闭源**：社区不满
